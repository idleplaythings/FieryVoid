MathLib =
{
    ...
    {
        var vector = new math.Vector3(vector.x, vector.y, 0);
        angle = MathLib.degreeToRadian(angle);
        var axis = new math.Vector3(0, 0, 1);
        var matrix = new math.Matrix4().makeRotationAxis(axis, -1 * angle);
    ...
    isBetweenPoints: function(a, b, thickness, point){
        var distance = MathLib.distance(a, b);
        var aToPoint = MathLib.distance(a, point);
        var bToPoint = MathLib.distance(b, point);

        if (aToPoint > distance*2 || bToPoint > distance*2)
            return;

        var maxDistance = thickness + Math.sqrt(Math.pow(distance, 2) + Math.pow(thickness, 2));

        return aToPoint + bToPoint < maxDistance;

    },

    getPointInDirectionInvertY: function( r, a, cx, cy){
    ...
    getExactPointInDirectionInvertY: function( r, a, cx, cy){
