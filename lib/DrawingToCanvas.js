...
model.DrawingToCanvas.prototype.drawBox = function(context, x, y, s, fill, padding, noStroke)
{
    if (! padding)
        padding = {
            top: 0,
    ...
    context.beginPath();
    context.moveTo(x + padding.left, y - padding.bottom);
    context.lineTo(x + s - padding.right , y - padding.bottom);
    context.lineTo(x+s - padding.right, y-s + padding.top);
    context.lineTo(x + padding.left, y-s + padding.top);
    context.lineTo(x + padding.left, y - padding.bottom);
...
model.DrawingToCanvas.prototype.scaleAndGetImageData = function(data, zoom)
{
    var width = data.width;
    var height = data.height;
	...
	var newCanvas2 = $("<canvas>")
        ...
        .attr("height", height*zoom)[0];
	
	var context = newCanvas2.getContext("2d");

    context.drawImage(newCanvas, 0, 0, width*zoom, height*zoom);
    return context.getImageData(0, 0, width*zoom, height*zoom);
};
...
model.DrawingToCanvas.prototype.createTexture = function(image)
{
    var texturedata = {
        data : new Uint8Array(
...
model.DrawingToCanvas.prototype.getImageDataFromImage = function(image)
{
